...
import type { MonthSummaryDto, HolidayDto } from '@/types/workCalendar';
...
/** Convert a local Date to YYYY-MM-DD key */
function toDateKey(date: Date): string {
  return `${date.getFullYear()}-${String(date.getMonth() + 1).padStart(2, '0')}-${String(date.getDate()).padStart(2, '0')}`;
}

/** Extract YYYY-MM-DD from an ISO date string (timezone-safe) */
...
export default function WorkCalendarPage() {
  ...
  // Helper sets for quick lookup (using YYYY-MM-DD keys to avoid timezone issues)
  ...
  const handleDayClick = (date: Date, isCurrentMonth: boolean) => {
    ...
    if (toggleMutation.isPending) return; // Prevent double-click
    // Send as YYYY-MM-DDT00:00:00 (no Z) to avoid timezone shift
