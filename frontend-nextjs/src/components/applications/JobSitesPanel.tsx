...
const SITES = [
  ...
  { name: 'Jobgether',        url: 'https://jobgether.com/remote-jobs/latam/net-developer', source: ApplicationSource.Jobgether },
...
export function JobSitesPanel({ conversionData = [] }: Props) {
  ...
  return (
    <div className="bg-card border border-border/60 rounded-2xl overflow-hidden shadow-sm">
      <button
        onClick={() => setOpen(v => !v)}
        className="w-full flex items-center justify-between px-4 py-3 hover:bg-muted/30 transition-colors focus:outline-none"
      >
        <div className="flex items-center gap-2">
          <Globe className="w-3.5 h-3.5 text-emerald-500" />
          <span className="text-xs font-semibold text-muted-foreground">Onde buscar vagas</span>
        </div>
        {open
        ...
        <div className="border-t border-border/60">
          {SITES.map(site => {
            ...
            return (
              ...
              >
                <span className="text-xs text-foreground group-hover:text-emerald-700 dark:group-hover:text-emerald-400 transition-colors">
                  {site.name}
                </span>
                <div className="flex items-center gap-2">
                  {count !== null && count > 0 && (
                    <span className="text-xs text-muted-foreground/60 tabular-nums bg-muted px-1.5 py-0.5 rounded-full">
                      {count}
                    </span>
                  )}
                  <ExternalLink className="w-3 h-3 text-muted-foreground/40 group-hover:text-emerald-500 transition-colors" />
                </div>
              </a>
            );
          })}
