...
namespace Orbit.Application.Features.Subscriptions.Queries;
    ...
    public async Task<SubscriptionForecastDto> Handle(GetSubscriptionForecastQuery request, CancellationToken cancellationToken)
    {
        ...
        foreach (var subscription in subscriptions)
        {
            ...
            if (subscription.IsLowUsage)
            {
                ...
                var isPaid = subscription.LastPaidAt.HasValue && subscription.LastPaidAt.Value.Month == now.Month && subscription.LastPaidAt.Value.Year == now.Year;
