...
using Orbit.Application.Features.Subscriptions.Queries;
using Orbit.Application.Features.Subscriptions.Commands;
...
public class SubscriptionsController : BaseAuthenticatedController
{
    ...
    public async Task<ActionResult<SubscriptionDto>> MarkPaid(Guid id)
    {
        var subscription = await _subscriptionRepository.GetByIdAsync(id, s => s.Category);
        if (subscription == null || subscription.FamilyId != FamilyId)
            return NotFound();

        subscription.LastPaidAt = DateTime.UtcNow;

        // Advance NextBillingDate if it's in the past or current month
        if (subscription.NextBillingDate.HasValue && subscription.NextBillingDate.Value <= DateTime.UtcNow)
        {
            subscription.NextBillingDate = subscription.NextBillingDate.Value.AddMonths(1);
        }
    ...
    private static SubscriptionDto MapToDto(Subscription s)
    {
        ...
        var isPaid = s.LastPaidAt.HasValue && s.LastPaidAt.Value.Month == now.Month && s.LastPaidAt.Value.Year == now.Year;
